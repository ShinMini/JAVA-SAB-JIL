...
public class Controller {
   ...
   private static Customer searchedCustomer = m.queryCustomer("철수");
   private static Product searchedProduct = m.queryProduct("로션");
   public static void reqRes(int no) {
      ...
      if (no == 1) { // 학생 정보 검색
         ...
         if (searchedCustomer.getName() != "error") {
            SuccessView.printCustomer(searchedCustomer);
         } else {
            FailView.printMsg("해당 학생은 존재하지 않습니다.");
         }
      } else if (no == 2) { // 물건 정보 검색

         SuccessView.printProductInfo(searchedProduct);
