...
public class ProbonoService {
	private static ProbonoDAO ProbonoDAO = new ProbonoDAO();
	private static ActivistDAO ActivistDAO = new ActivistDAO();

	// Probono - CRUD
	public static void notExistProbono(int probonoId) throws NotExistException, SQLException {
	...
	// 모든 probono 정보 반환
	public static List<ProbonoDTO> getAllProbonos() throws SQLException {
		List<ProbonoDTO> probonos = null;
		try {
			probonos = ProbonoDAO.getAllProbonos();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return probonos;
	...
	// probono id로 검색
	public static ProbonoDTO getProbono(int probonoId) throws SQLException, NotExistException {
		ProbonoDTO probono = null;
		try {
			probono = ProbonoDAO.getProbono(probonoId);
			if (probono == null) {
	...
	public static boolean addProbono(ProbonoDTO probono) throws SQLException {
		...
		try {
			checker = ProbonoDAO.createProbono(probono);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return checker;
	...
	// 기존 probono 수정
	public static boolean updateProbono(int probonoId, String probonoPurpose) throws SQLException, NotExistException {
		boolean checker = false;
		notExistProbono(probonoId);
		try {
			checker = ProbonoDAO.updateProbonoPurpose(probonoId, probonoPurpose);
	...
	// probono 삭제
	public boolean deleteProbono(int probonoId) throws SQLException, NotExistException {
		boolean checker = false;
		notExistProbono(probonoId);
		try {
			checker = ProbonoDAO.deleteProbono(probonoId);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return checker;
	...
	// Activist - CRUD
	public static void notExistActivist(int activistId) throws NotExistException, SQLException {
		ActivistDTO activist;
		try {
			activist = ActivistDAO.getActivist(activistId);
			if (activist == null) {
	...
	public static boolean addActivist(ActivistDTO activist) throws MessageException {
		...
		try {
			result = ActivistDAO.createActivist(activist);
	...
	public static boolean updateActivist(int activistId, String major) throws SQLException, NotExistException {
		notExistActivist(activistId);
		boolean result = false;
		try {
			result = ActivistDAO.updateActivist(activistId, major);
			if (!result) {
	...
	public static boolean deleteActivist(int activistId) throws SQLException, NotExistException {
		notExistActivist(activistId);
		boolean result = false;
		try {
			result = ActivistDAO.deleteActivist(activistId);
			if (!result) {
	...
	public static ActivistDTO getActivist(int activistId) throws SQLException, NotExistException {
	...
	public static List<ActivistDTO> getAllActivists() throws SQLException {
		List<ActivistDTO> activists = null;
		try {
			activists = ActivistDAO.getAllActivists();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return activists;
	...
	// Recipient - CRUD
	public static void notExistRecipient(int recipientId) throws NotExistException, SQLException {
	...
	public static boolean updateRecipient(int recipientId, String receiveHopeContent)
	...
	public boolean deleteRecipient(int recipientId) throws SQLException, NotExistException {
		notExistRecipient(recipientId);
		return RecipientDAO.deleteRecipient(recipientId);
	}

	public static RecipientDTO getRecipient(int recipientId) throws SQLException {
	...
	public static boolean updateProbonoProjectActivist(int probonoProjectId, String activistId)
			throws SQLException, NotExistException {
